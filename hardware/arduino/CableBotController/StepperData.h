...
class StepperData {
  public:        
    ...
    StepperData operator*(StepperData other) {StepperData res; for (uint8_t i=0;i<N_MOTORS;i++) res._array[i] = _array[i]*other._array[i];return res;};
    StepperData operator/(StepperData other) {StepperData res; for (uint8_t i=0;i<N_MOTORS;i++) res._array[i] = _array[i]/other._array[i];return res;};
    bool operator==(StepperData other) {for (uint8_t i=0;i<N_MOTORS;i++) if(_array[i] != other._array[i]) return false;return true;};
    // Printing function
    String to_String();
...
void testOperationsOnStepperPositions() {
  ...
  Serial.println(x0.to_String() + " * " + x1.to_String() + " = " + (x0*x1).to_String());
  Serial.println(x0.to_String() + " / " + x1.to_String() + " = " + (x0/x1).to_String());
  Serial.println(x0.to_String() + " == " + x0.to_String() + " = " + String(x0==x0));
  Serial.println(x0.to_String() + " == " + x1.to_String() + " = " + String(x0==x1));
};
